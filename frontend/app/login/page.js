  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'} flex items-center justify-center p-4 transition-colors`}>
      ...
      <div className={`${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-purple-100'} rounded-2xl shadow-xl p-8 w-full max-w-md border-2 transition-colors`}>
        ...
        <div className="mb-8">
          <h1 className={`text-3xl font-bold ${theme === 'dark' ? 'text-[#e55753]' : 'text-gray-800'} mb-2`}>Welcome Back</h1>
          <p className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Login to CodeMentor</p>
        </div>

        {errMsg ? (
          <div className={`mb-6 ${theme === 'dark' ? 'bg-red-900 border-red-700 text-red-200' : 'bg-red-100 border-red-400 text-red-700'} border px-4 py-3 rounded-lg`}>
            {errMsg}
          </div>
        ) : null}
        ...
        <p className={`text-center ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mt-6`}>
          ...
          <Link href="/signup" className={`font-semibold hover:underline ${theme === 'dark' ? 'text-purple-400' : 'text-purple-600'}`}>Sign Up</Link>
        </p>
      </div>
    </div>
  );
